...
export default function CreateListing() { 
 ...
 const {currentuser} = useSelector((state)=>state.user)
  ...
  const handleSubmit=async(e)=>{
    e.preventDefault() 
    try{
      if(formData.imageUrls.length<1 )return setError('You mush have upload atleat 1 image') 
  ...
  const storeImage = async (file) => {
    return new Promise((resolve, reject) => {
      ...
      const fileName = new Date().getTime() + "_" + file.name; // Use an underscore
      console.log(fileName, "Generated file name");
      const storageRef = ref(storage, `uploads/${fileName}`); // Specify the path
        ...
        <div className="flex flex-col flex-1 gap-4">
          ...
          {formData.imageUrls.length > 0 &&
            formData.imageUrls.map((url, index) => (
              ...
              className="text-red-700 uppercase p-3">delete</button>
              </div>
            ))}
